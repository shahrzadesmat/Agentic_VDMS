...
def run_optimization(method: str, iterations: int, port: int, dataset_dir: Path,
                     ...
                     t_exp_frac: float = 0.40,
                     t_expl_frac: float = 0.75,
                     nsga2_population: int = 10) -> List[IterationResult]:
    ...
    if method == "optuna":
        ...
        # Evolutionary baseline (Deb et al., NSGA-II). Single-objective on the
        # SIEVE Score, exactly as optuna/gp_bo, so Table 4 stays apples-to-apples.
        #
        # population_size MUST be set: Optuna's default is 50, which with a
        # 50-iteration budget yields a single generation — the whole budget is
        # spent on the random initial population and no crossover or mutation
        # ever runs, silently reducing NSGA-II to random search. At the default
        # 10 the budget covers ~5 generations.
        if nsga2_population >= iterations:
            print(f"[NSGA-II][WARN] population_size={nsga2_population} >= "
                  f"iterations={iterations}: only one generation will run, so this "
                  f"is effectively random search. Use a smaller population.")
        print(f"[NSGA-II] population_size={nsga2_population} "
              f"(~{max(1, iterations // max(1, nsga2_population))} generations "
              f"over {iterations} iterations)")
        optuna_study = optuna.create_study(
            direction="maximize",
            sampler=optuna.samplers.NSGAIISampler(
                seed=seed, population_size=nsga2_population),
        )
    ...
    for i in range(iterations):
        ...
        elif method in ("optuna", "gp_bo", "nsga2"):
            opt_tried_keys = {
                (r.config["engine"],
                 str(sorted(r.config.get("params", {}).items())),
                 r.config.get("k_neighbors", 500),
        ...
        try:
            ...
            if method in ("optuna", "gp_bo", "nsga2") and optuna_trial is not None:
...
def main():
    ...
    parser.add_argument("--method",
                        choices=["hyperparameter_only", "random", "grid", "optuna", "gp_bo", "vdtuner", "nsga2"],
                        required=True)
    parser.add_argument("--nsga2-population", type=int, default=10,
                        help="NSGA-II population size (default 10 -> ~5 generations "
                             "at 50 iterations). Optuna's own default of 50 would "
                             "give a single generation, i.e. random search.")
