...
class LLMProvider:
    def __init__(self, api_key: str, model: str = "minimax/minimax-m2.1",
                 ...
                 ablation_no_guidance: bool = False,
                 ablation_no_hints: bool = False,
        ...
        self.ablation_no_guidance = ablation_no_guidance  # Ablation C: drop diagnostic class g(H)
        self.ablation_no_hints    = ablation_no_hints     # Ablation D: drop untried-value hints
...
"""
        ...
        # Ablation C: drop the diagnostic guidance class g(H). History, phases and
        # hints are untouched, so the agent still sees every past (config, Score)
        # pair -- it simply is not told which failure mode the last run exhibited.
        if self.ablation_no_guidance:
            guidance = "(ABLATION: diagnostic guidance disabled)"

        # Ablation D: drop the untried-value hints. The agent keeps history, phases
        # and diagnosis, but is no longer handed the enumerated untried alpha /
        # (n_refs, ref_strategy) combinations at the incumbent best configuration.
        if self.ablation_no_hints:
            untried_alpha_hint = ""
            untried_refs_hint  = ""
...
def run_optimization(method: str, iterations: int, port: int, dataset_dir: Path,
    ...
    llm_provider    = LLMProvider(api_key, model=model, backbone=backbone, seed=seed,
                                  ...
                                  ablation_no_guidance=ablation_no_guidance,
                                  ablation_no_hints=ablation_no_hints,
...
def main():
    ...
    parser.add_argument("--ablation-no-guidance", action="store_true",
                        help="Ablation C: drop the diagnostic guidance class g(H) from the prompt; history, phases and hints retained.")
    parser.add_argument("--ablation-no-hints", action="store_true",
                        help="Ablation D: drop untried-value hints (alpha and n_refs/ref_strategy) from the prompt; history, phases and guidance retained.")
    ...
    results = run_optimization(
        ...
        ablation_no_guidance=  args.ablation_no_guidance,
        ablation_no_hints=     args.ablation_no_hints,
