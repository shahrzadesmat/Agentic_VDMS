...
# ---------------------------------------------------------------------------
# ECI — constrained Bayesian optimization baseline
#   Gardner et al., "Bayesian Optimization with Inequality Constraints",
#   ICML 2014, PMLR 32:937-945.  Optuna composes an objective GP with a
#   feasibility GP as ConstrainedLogEI.
#
# Contract (differs from gp_bo on purpose):
#   objective  = RAW QPS          (smooth; NOT the cliffed SIEVE Score)
#   constraint = tau - mAP        (Optuna convention: feasible iff c <= 0)
# Telling the cliffed Score here would make ECI degenerate into gp_bo.
# ---------------------------------------------------------------------------
_ECI_CONSTRAINT_KEY = "eci_c"


def _feasibility_map(bm: "BenchmarkResult") -> float:
    """The mAP quantity score() thresholds against — mirrors score() exactly.

    Kept in lockstep with BenchmarkResult.score(): under QDS the gate uses the
    difficulty-weighted mAP, so a constraint built on plain map_score would
    optimize a different feasible set than the one reported.
    """
    if _QDS_WEIGHTS is not None and bm.per_query_ap and not _QDS_STANDARD_THRESHOLD:
        w = _QDS_WEIGHTS
        n = min(len(w), len(bm.per_query_ap))
        return float(np.sum(w[:n] * np.array(bm.per_query_ap[:n])) / np.sum(w[:n]))
    return float(bm.map_score)


def _eci_constraint_value(bm: "BenchmarkResult") -> float:
    """c = tau - mAP.  Feasible iff c <= 0."""
    if _NO_THRESHOLD:
        return -1.0          # gate disabled: every config is feasible
    return float(_MAP_THRESHOLD - _feasibility_map(bm))


def _eci_constraints_func(trial) -> Tuple[float]:
    """Read the value stashed on the trial at tell time.

    Fails loud rather than defaulting: a missing constraint would either crash
    later inside Optuna ("The number of constraints must be the same for all
    trials") or, with a silent default, bias the feasibility GP. Every
    study.tell() on an ECI run must set the attr first.
    """
    if _ECI_CONSTRAINT_KEY not in trial.user_attrs:
        raise RuntimeError(
            f"ECI: trial {trial.number} completed without a constraint value. "
            f"Set trial.set_user_attr('{_ECI_CONSTRAINT_KEY}', tau - mAP) "
            "before every study.tell() on an ECI run."
        )
    return (float(trial.user_attrs[_ECI_CONSTRAINT_KEY]),)
...
def run_optimization(method: str, iterations: int, port: int, dataset_dir: Path,
    ...
    if method == "optuna":
        ...
        # Constrained BO (Gardner et al., ICML'14). Same GP backend and same
        # n_startup_trials as gp_bo so the only difference is the constraint.
        optuna_study = optuna.create_study(
            direction="maximize",
            sampler=optuna.samplers.GPSampler(
                seed=seed, constraints_func=_eci_constraints_func),
        )
    ...
    for i in range(iterations):
        ...
        elif method in ("optuna", "gp_bo", "eci"):
            opt_tried_keys = {
                (r.config["engine"],
                 str(sorted(r.config.get("params", {}).items())),
                 r.config.get("k_neighbors", 500),
                ...
                dup_res = next(
                    (r for r in results if
                     (r.config["engine"],
                      ...
                      r.config.get("n_refs", 1),
                      r.config.get("ref_strategy", "first"),
                      r.config.get("constraint_strategy", "none")) == opt_key), None)
                if method == "eci":
                    # This branch tells Optuna without running a benchmark, so the
                    # constraint must be supplied from the earlier observation of
                    # the same config; otherwise _eci_constraints_func raises.
                    if dup_res is not None:
                        optuna_trial.set_user_attr(
                            _ECI_CONSTRAINT_KEY, _eci_constraint_value(dup_res.benchmark))
                        dup_score = float(dup_res.benchmark.qps)
                    else:
                        optuna_trial.set_user_attr(_ECI_CONSTRAINT_KEY, 1.0)  # infeasible
                        dup_score = 0.0
                else:
                    dup_score = dup_res.benchmark.score() if dup_res is not None else 0.0
        ...
        try:
            ...
            if method in ("optuna", "gp_bo") and optuna_trial is not None:
                ...
                optuna_trial = None
            elif method == "eci" and optuna_trial is not None:
                # Objective = raw QPS (smooth); feasibility carried by the
                # constraint GP. Reported Score in results/ is still SIEVE, so
                # the run stays directly comparable to every other method.
                optuna_trial.set_user_attr(
                    _ECI_CONSTRAINT_KEY, _eci_constraint_value(benchmark))
                optuna_study.tell(optuna_trial, float(benchmark.qps))
                optuna_trial = None
            if method == "vdtuner":
            ...
            if method in ("optuna", "gp_bo") and optuna_trial is not None:
                ...
                # A crashed benchmark is not an infeasible config. Mark FAIL so
                # the trial is excluded from the GP (only COMPLETE trials are
                # modelled) and no constraint is required for it.
...
def main():
    ...
    parser.add_argument("--method",
                        choices=["hyperparameter_only", "random", "grid", "optuna", "gp_bo", "vdtuner", "eci"],
                        required=True)
