#!/bin/bash
#SBATCH --job-name=hico_vdt_cB_s200
...
#SBATCH --output=hico_det/conditionB/hico_vdt_cB_s200_%j.log
#SBATCH --error=hico_det/conditionB/hico_vdt_cB_s200_%j.err
...
PORT=55685
INSTANCE="vdms_vdt_cB_s200_${SLURM_JOB_ID}"
...
DB_ROOT="/tmp/vdms_vdt_cB_s200_${SLURM_JOB_ID}"
VDMS_CFG="/tmp/vdms_vdt_cB_s200_${SLURM_JOB_ID}.json"
RESULTS_BASE="/work/hdd/bdjd/vdms_workflow/semantic_vdms/supplementary/results/hico_det_condB"
OUTPUT="${RESULTS_BASE}/vdtuner/seed200.json"

echo "===== HICO-DET VDTuner ConditionB seed=200 ====="
echo "Job: ${SLURM_JOB_ID}  Node: $(hostname)  Started: $(date)"

mkdir -p "${RESULTS_BASE}/vdtuner"
...
start_vdms() {
    ...
    apptainer exec instance://${INSTANCE} \
        ...
        > /tmp/vdms_vdt_cB_s200_${SLURM_JOB_ID}.log 2>&1 &
