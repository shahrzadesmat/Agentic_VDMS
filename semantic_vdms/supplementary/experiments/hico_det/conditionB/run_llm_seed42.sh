#!/bin/bash
#SBATCH --job-name=hico_llm_cB_s42
...
#SBATCH --output=hico_det/conditionB/hico_llm_cB_s42_%j.log
#SBATCH --error=hico_det/conditionB/hico_llm_cB_s42_%j.err
#SBATCH --chdir=/work/hdd/bdjd/vdms_workflow/semantic_vdms/supplementary/experiments

# Condition B: multi-row VDMS indexing with all_verb_ids (one descriptor per image×verb_id pair).
# ~90,200 descriptors instead of 47,776. Dedup + k-inflation applied at query time.

export OPENROUTER_API_KEY="${OPENROUTER_API_KEY:-YOUR_OPENROUTER_API_KEY}"

PORT=55670
INSTANCE="vdms_llm_cB_s42_${SLURM_JOB_ID}"
...
DB_ROOT="/tmp/vdms_llm_cB_s42_${SLURM_JOB_ID}"
VDMS_CFG="/tmp/vdms_llm_cB_s42_${SLURM_JOB_ID}.json"
RESULTS_BASE="/work/hdd/bdjd/vdms_workflow/semantic_vdms/supplementary/results/hico_det_condB"
OUTPUT="${RESULTS_BASE}/llm/seed42.json"

echo "===== HICO-DET LLM ConditionB seed=42 ====="
echo "Job: ${SLURM_JOB_ID}  Node: $(hostname)  Started: $(date)"

mkdir -p "${RESULTS_BASE}/llm"
...
start_vdms() {
    ...
    apptainer exec instance://${INSTANCE} \
        ...
        > /tmp/vdms_llm_cB_s42_${SLURM_JOB_ID}.log 2>&1 &
