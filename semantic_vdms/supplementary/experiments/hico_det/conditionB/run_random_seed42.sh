#!/bin/bash
#SBATCH --job-name=hico_rand_cB_s42
...
#SBATCH --output=hico_det/conditionB/hico_rand_cB_s42_%j.log
#SBATCH --error=hico_det/conditionB/hico_rand_cB_s42_%j.err
...
# Using gpuA40x4 (not interactive) — indexing ~90K descriptors needs more than 59 min.

PORT=55674
INSTANCE="vdms_rand_cB_s42_${SLURM_JOB_ID}"
...
DB_ROOT="/tmp/vdms_rand_cB_s42_${SLURM_JOB_ID}"
VDMS_CFG="/tmp/vdms_rand_cB_s42_${SLURM_JOB_ID}.json"
RESULTS_BASE="/work/hdd/bdjd/vdms_workflow/semantic_vdms/supplementary/results/hico_det_condB"
OUTPUT="${RESULTS_BASE}/random/seed42.json"

echo "===== HICO-DET Random ConditionB seed=42 ====="
echo "Job: ${SLURM_JOB_ID}  Node: $(hostname)  Started: $(date)"

mkdir -p "${RESULTS_BASE}/random"
...
start_vdms() {
    ...
    apptainer exec instance://${INSTANCE} \
        ...
        > /tmp/vdms_rand_cB_s42_${SLURM_JOB_ID}.log 2>&1 &
