#!/bin/bash
#SBATCH --job-name=hico_grid_cB_s42
...
#SBATCH --output=hico_det/conditionB/hico_grid_cB_s42_%j.log
#SBATCH --error=hico_det/conditionB/hico_grid_cB_s42_%j.err
...
# Grid search is fully deterministic — one seed is sufficient.

PORT=55673
INSTANCE="vdms_grid_cB_s42_${SLURM_JOB_ID}"
...
DB_ROOT="/tmp/vdms_grid_cB_s42_${SLURM_JOB_ID}"
VDMS_CFG="/tmp/vdms_grid_cB_s42_${SLURM_JOB_ID}.json"
RESULTS_BASE="/work/hdd/bdjd/vdms_workflow/semantic_vdms/supplementary/results/hico_det_condB"
OUTPUT="${RESULTS_BASE}/grid/seed42.json"

echo "===== HICO-DET Grid ConditionB seed=42 ====="
echo "Job: ${SLURM_JOB_ID}  Node: $(hostname)  Started: $(date)"

mkdir -p "${RESULTS_BASE}/grid"
...
start_vdms() {
    ...
    apptainer exec instance://${INSTANCE} \
        ...
        > /tmp/vdms_grid_cB_s42_${SLURM_JOB_ID}.log 2>&1 &
