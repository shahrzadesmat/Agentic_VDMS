#!/bin/bash
#SBATCH --job-name=gldv2_gpbo_s200
...
#SBATCH --output=/work/hdd/bdjd/vdms_workflow/semantic_vdms/gldv2/gldv2_gpbo_s200_%j.log
#SBATCH --error=/work/hdd/bdjd/vdms_workflow/semantic_vdms/gldv2/gldv2_gpbo_s200_%j.err

# GLDv2 GPBO — 50 iterations, seed=200

export OPENROUTER_API_KEY="${OPENROUTER_API_KEY:-YOUR_KEY_HERE}"

SEED=200
PORT=55689
INSTANCE="vdms_gldv2_gpbo_s200_${SLURM_JOB_ID}"
...
DB_ROOT="/tmp/vdms_gldv2_gpbo_s200_${SLURM_JOB_ID}"
VDMS_CFG="/tmp/vdms_gldv2_gpbo_s200_${SLURM_JOB_ID}.json"
OUTPUT="${BASE_DIR}/supplementary/results/gldv2/gpbo/seed200.json"

echo "===== GLDv2 gpbo seed=200 ====="
...
start_vdms() {
    ...
    apptainer exec instance://${INSTANCE} \
        /vdms/build/vdms -cfg "$VDMS_CFG" > /tmp/vdms_gldv2_gpbo_s200_${SLURM_JOB_ID}.log 2>&1 &
    ...
    if [ $? -ne 0 ]; then
        ...
        cat /tmp/vdms_gldv2_gpbo_s200_${SLURM_JOB_ID}.log
