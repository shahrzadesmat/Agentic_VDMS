#!/bin/bash
#SBATCH --job-name=gldv2_grid_s200
...
#SBATCH --output=/work/hdd/bdjd/vdms_workflow/semantic_vdms/gldv2/gldv2_grid_s200_%j.log
#SBATCH --error=/work/hdd/bdjd/vdms_workflow/semantic_vdms/gldv2/gldv2_grid_s200_%j.err

PORT=55645
INSTANCE="vdms_gldv2_grid_s200_${SLURM_JOB_ID}"
...
DB_ROOT="/tmp/vdms_gldv2_grid_s200_${SLURM_JOB_ID}"
VDMS_CFG="/tmp/vdms_gldv2_grid_s200_${SLURM_JOB_ID}.json"
FINAL_RESULTS="${BASE_DIR}/FINAL_RESULTS/gldv2/results"
OUTPUT="${FINAL_RESULTS}/grid_seed200.json"

echo "===== GLDv2 Grid seed=200 ====="
echo "Job: ${SLURM_JOB_ID}  Node: $(hostname)  Started: $(date)"

mkdir -p "$FINAL_RESULTS"
OPTIMIZER="/work/hdd/bdjd/vdms_workflow/src/gldv2/gldv2_agent_optimizer.py"
cd "$BASE_DIR/gldv2"
...
start_vdms() {
    ...
    apptainer exec instance://${INSTANCE} \
        /vdms/build/vdms -cfg "$VDMS_CFG" > /tmp/vdms_gldv2_grid_s200_${SLURM_JOB_ID}.log 2>&1 &
