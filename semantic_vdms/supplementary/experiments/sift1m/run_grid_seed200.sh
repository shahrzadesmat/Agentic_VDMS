#!/bin/bash
#SBATCH --job-name=sift1m_grid_s200
...
#SBATCH --output=/work/hdd/bdjd/vdms_workflow/semantic_vdms/sift1m/sift1m_grid_s200_%j.log
#SBATCH --error=/work/hdd/bdjd/vdms_workflow/semantic_vdms/sift1m/sift1m_grid_s200_%j.err

SEED=200
PORT=55662
INSTANCE="vdms_sift1m_grid_s200_${SLURM_JOB_ID}"
...
DB_ROOT="/tmp/vdms_sift1m_grid_s200_${SLURM_JOB_ID}"
VDMS_CFG="/tmp/vdms_sift1m_grid_s200_${SLURM_JOB_ID}.json"
SNAP_KEY_FILE="/tmp/vdms_snap_key_sift1m_grid_s200_${SLURM_JOB_ID}.txt"
VDMS_PARAMS_FILE="/tmp/vdms_params_sift1m_grid_s200_${SLURM_JOB_ID}.json"
FINAL_RESULTS="${BASE_DIR}/FINAL_RESULTS/sift1m/results"
OUTPUT="${FINAL_RESULTS}/grid_seed200.json"
OPTIMIZER="/work/hdd/bdjd/vdms_workflow/src/sift1m/sift1m_agent_optimizer.py"
export SNAP_KEY_FILE VDMS_PARAMS_FILE

echo "===== SIFT1M Grid seed=200 ====="
...
"
    ...
    apptainer exec instance://${INSTANCE} \
        /vdms/build/vdms -cfg "$VDMS_CFG" > /tmp/vdms_sift1m_grid_s200_${SLURM_JOB_ID}.log 2>&1 &
    timeout 180 bash -c "until nc -z localhost ${PORT}; do sleep 1; done" 2>/dev/null
    if [ $? -ne 0 ]; then
        echo "ERROR: VDMS did not become ready"
...
PYTHONUNBUFFERED=1 $PYTHON "$OPTIMIZER" \
    --port "$PORT" \
