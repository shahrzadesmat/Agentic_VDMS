#!/bin/bash
#SBATCH --job-name=hico_opt_r_s99
...
#SBATCH --output=hico_det/hico_opt_r_s99_%j.log
#SBATCH --error=hico_det/hico_opt_r_s99_%j.err

export OPENROUTER_API_KEY="${OPENROUTER_API_KEY:-YOUR_KEY_HERE}"

PORT=55635
INSTANCE="vdms_opt_r_s99_${SLURM_JOB_ID}"
...
DB_ROOT="/tmp/vdms_opt_r_s99_${SLURM_JOB_ID}"
VDMS_CFG="/tmp/vdms_opt_r_s99_${SLURM_JOB_ID}.json"
FINAL_RESULTS="/work/hdd/bdjd/vdms_workflow/semantic_vdms/FINAL_RESULTS/hico_det/results"
OUTPUT="${FINAL_RESULTS}/optuna_seed99.json"

echo "===== Optuna TPE rerun seed=99 ====="
...
start_vdms() {
    ...
    apptainer exec instance://${INSTANCE} \
        ...
        > /tmp/vdms_opt_r_s99_${SLURM_JOB_ID}.log 2>&1 &
