...
#SBATCH --output=/work/hdd/bdjd/vdms_workflow/semantic_vdms/hico_det/no_threshold/logs/nt_qds_grid_%j.log
#SBATCH --error=/work/hdd/bdjd/vdms_workflow/semantic_vdms/hico_det/no_threshold/logs/nt_qds_grid_%j.err

set -euo pipefail
export PYTHONUNBUFFERED=1

PORT=55673
INSTANCE="vdms_nothresh_nt_qds_grid_${SLURM_JOB_ID}"
...
DB_ROOT="/tmp/vdms_nothresh_nt_qds_grid_${SLURM_JOB_ID}"
VDMS_CFG="/tmp/vdms_nothresh_nt_qds_grid_${SLURM_JOB_ID}.json"
RESULTS="${BASE_DIR}/hico_det/no_threshold/results"
OUTPUT="${RESULTS}/nothresh_qds_grid.json"

echo "===== nt_qds_grid (no-threshold ablation) ====="
...
start_vdms() {
    ...
    apptainer exec instance://${INSTANCE} \
        ...
        > /tmp/vdms_nothresh_nt_qds_grid_${SLURM_JOB_ID}.log 2>&1 &
