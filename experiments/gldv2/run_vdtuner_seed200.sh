#!/bin/bash
#SBATCH --job-name=gldv2_vdt_s200
...
#SBATCH --output=/work/hdd/bdjd/vdms_workflow/semantic_vdms/gldv2/gldv2_vdt_s200_%j.log
#SBATCH --error=/work/hdd/bdjd/vdms_workflow/semantic_vdms/gldv2/gldv2_vdt_s200_%j.err

# GLDv2 VDTuner (EHVI multi-objective BO) — 50 iterations, seed=42

SEED=200
PORT=55705
INSTANCE="vdms_gldv2_vdt_s200_${SLURM_JOB_ID}"
...
DB_ROOT="/tmp/vdms_gldv2_vdt_s200_${SLURM_JOB_ID}"
VDMS_CFG="/tmp/vdms_gldv2_vdt_s200_${SLURM_JOB_ID}.json"
OUTPUT="${BASE_DIR}/supplementary/results/gldv2/vdtuner/seed200.json"
...
start_vdms() {
    ...
    apptainer exec instance://${INSTANCE} \
        /vdms/build/vdms -cfg "$VDMS_CFG" > /tmp/vdms_gldv2_vdt_s200_${SLURM_JOB_ID}.log 2>&1 &
    ...
    if [ $? -ne 0 ]; then
        ...
        cat /tmp/vdms_gldv2_vdt_s200_${SLURM_JOB_ID}.log
