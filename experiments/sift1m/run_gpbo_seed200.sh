#!/bin/bash
#SBATCH --job-name=sift1m_gpbo_s200
...
#SBATCH --output=/work/hdd/bdjd/vdms_workflow/semantic_vdms/sift1m/sift1m_gpbo_s200_%j.log
#SBATCH --error=/work/hdd/bdjd/vdms_workflow/semantic_vdms/sift1m/sift1m_gpbo_s200_%j.err

# SIFT1M GPBO — 50 iterations, seed=200

export OPENROUTER_API_KEY="${OPENROUTER_API_KEY:-YOUR_KEY_HERE}"

SEED=200
PORT=55699
INSTANCE="vdms_sift1m_gpbo_s200_${SLURM_JOB_ID}"
...
DB_ROOT="/tmp/vdms_sift1m_gpbo_s200_${SLURM_JOB_ID}"
VDMS_CFG="/tmp/vdms_sift1m_gpbo_s200_${SLURM_JOB_ID}.json"
SNAP_KEY_FILE="/tmp/vdms_snap_key_sift1m_gpbo_s200_${SLURM_JOB_ID}.txt"
VDMS_PARAMS_FILE="/tmp/vdms_params_sift1m_gpbo_s200_${SLURM_JOB_ID}.json"
OUTPUT="${BASE_DIR}/supplementary/results/sift1m/gpbo/seed200.json"
export SNAP_KEY_FILE VDMS_PARAMS_FILE

echo "===== SIFT1M gpbo seed=200 ====="
...
"
    ...
    apptainer exec instance://${INSTANCE} \
        /vdms/build/vdms -cfg "$VDMS_CFG" > /tmp/vdms_sift1m_gpbo_s200_${SLURM_JOB_ID}.log 2>&1 &
