...
#SBATCH --output=/work/hdd/bdjd/vdms_workflow/semantic_vdms/sift1m/sift1m_grid_rerun_%j.log
#SBATCH --error=/work/hdd/bdjd/vdms_workflow/semantic_vdms/sift1m/sift1m_grid_rerun_%j.err

# SIFT1M Grid Baseline — RERUN with constrained score (QPS if Recall@10 ≥ 0.90 else 0).

SEED=42
PORT=55660
INSTANCE="vdms_sift1m_grid_r_${SLURM_JOB_ID}"
...
DB_ROOT="/tmp/vdms_sift1m_grid_r_${SLURM_JOB_ID}"
VDMS_CFG="/tmp/vdms_sift1m_grid_r_${SLURM_JOB_ID}.json"
SNAP_KEY_FILE="/tmp/vdms_snap_key_sift1m_grid_r_${SLURM_JOB_ID}.txt"
VDMS_PARAMS_FILE="/tmp/vdms_params_sift1m_grid_r_${SLURM_JOB_ID}.json"
...
echo "========================================="
echo "SIFT1M Grid Baseline — RERUN"
echo "  Method:     grid (30 fixed configs)"
...
"
    ...
    apptainer exec instance://${INSTANCE} \
        /vdms/build/vdms -cfg "$VDMS_CFG" > /tmp/vdms_sift1m_grid_r_${SLURM_JOB_ID}.log 2>&1 &
    ...
    if [ $? -ne 0 ]; then
        ...
        cat /tmp/vdms_sift1m_grid_r_${SLURM_JOB_ID}.log
