...
proceeds in two stages, as shown in Figure~\ref{fig:pipeline}.
...
$\delta_j^c = \|\mathbf{c}_j - \mathbf{q}_c\|_2$, forming the candidate pool
\begin{equation}
  \mathcal{T}_k(q) = \operatorname{argtop\text{-}}k_{j \in [N]}\;
  \|\mathbf{c}_j - \mathbf{q}_c\|_2 ,
  \label{eq:stage1}
\end{equation}
sorted in ascending order of L2 distance.
...
DINOv2 ViT-L/14-reg4 encodes low-level visual geometry --- body pose, object shape,
and spatial arrangement --- that is complementary to CLIP's semantic alignment.
...
CLIP's contrastive training objective pulls the text embedding of any category label
toward the mean visual embedding of all matching images~\cite{radford2021clip}.
On a compositionally structured label such as \textit{``a person riding a horse''} ---
which aggregates images spanning diverse viewpoints, backgrounds, and subjects --- this
means $\mathbf{q}_c \approx \boldsymbol{\mu}_h^c$, making the bias term in
...
This advantage grows with the compositional difficulty of the category: when many HOI
categories share the same object class (e.g., \textit{ride}, \textit{sit\_on},
\textit{straddle} applied to the same objects), the intra-class variance $\sigma_h^2$
is large and reference-image displacement $\varepsilon_j$ is substantial relative to
inter-category distance, further penalizing image queries.
