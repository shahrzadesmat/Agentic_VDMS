...
\caption{Comparison of ANN index hyperparameter optimization methods on HICO-DET (30 iterations, best config reported).
  ...
  The LLM agent achieves the highest Score and reaches Score $= 3.86$ within 5 iterations ---
  a level requiring 9 iterations for Optuna TPE and 28 for Random Search.
...
\midrule
Grid Search$^\dagger$         & 3.027          & 0.148          & ---            & 20.49          & 7/12  \\
Random Search                 & 4.095          & 0.200          & 0.449          & 20.47          & 28/30 \\
Optuna TPE                    & 4.113          & 0.258          & 0.502          & 15.97          & 27/30 \\
\textbf{LLM Agent (Ours)}     & \textbf{4.337} & 0.255          & 0.489          & \textbf{16.98} & \textbf{30/30} \\
\bottomrule
\end{tabular}
\end{table*}
...
over the CLIP FaissFlat baseline at $k{=}500$ (Score $= 1.707$) and $\mathbf{13.6\times}$
...
scores, trailing the LLM agent by $5.2\%$ and $5.6\%$ respectively.
Grid Search (Score $= 3.027$) performs substantially worse, early-stopping at
iteration~12 and achieving mAP only $0.148$ --- less than half the CLIP-only exact-search
baseline at $k{=}2000$.

\noindent\textbf{Sample efficiency.}
The LLM agent's primary advantage lies in \emph{sample efficiency}: it reaches Score $= 3.86$
...
Optuna TPE first exceeds the LLM agent's iteration-5 threshold at iteration~9
...
concrete practical savings: the LLM agent reaches Score~$\geq 3.86$ approximately
$30$ minutes ahead of Optuna TPE (four fewer evaluations) and over $2.5$ hours ahead of
...
$(M, k, \alpha, n_r, \text{ref\_strategy})$ cannot be reached within the
early-stopping horizon.
...
The Score contrast is stark: $4.337$ and $4.113$ for HNSW-based methods vs.\ $3.027$ for
