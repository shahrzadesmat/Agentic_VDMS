...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.72\textwidth]{figures/convergence_hico_det.pdf}
...
All optimization results represent a single run per method; statistical confidence
intervals over multiple independent runs have not been quantified.
...
On the optimization side, we presented the first LLM agent for ANN index hyperparameter
optimization in a vector database setting.
Operating over a joint search space of approximately $102{,}000$ configurations, the agent
observes the full history of (configuration, Score) pairs and reasons about engine
architecture, graph connectivity, search depth, and fusion weight as an integrated system.
Its phase-aware schedule --- broad structural exploration followed by exploitation of the
best engine--$M$ pair followed by fine-grained continuous refinement --- encodes the
conditional dependency structure of the search space in a form that neither Bayesian
optimizers with parameter-independent marginals nor non-adaptive baselines can replicate.
The result is strong sample efficiency: the LLM agent reaches Score $= 3.86$ within
5 iterations, a threshold that requires 9 iterations for Optuna TPE and 28 for Random
Search, saving approximately $30$ minutes and $2.5$ hours of compute respectively.
The optimized pipeline achieves Score $= 4.337$ ($\text{mAP} \times \text{QPS}$), a
$\mathbf{2.5\times}$ improvement over the CLIP FaissFlat baseline and $\mathbf{13.6\times}$
over brute-force CLIP$+$DINOv2 reranking, at only $2.1$ percentage points of mAP below
the exact-search upper bound --- a $14.7\times$ throughput gain (QPS $= 16.98$ vs.\
$1.15$) that the brute-force baseline cannot approach.
...
Second, running each optimization method over multiple independent seeds will quantify
statistical uncertainty in the sample efficiency and final Score comparisons.
